...
def coords(tomato_boxes, color_image, result_image, profile, outs, aligned_depth_frame, frameset): #,depth_frame, frameset,aligned_depth_frame):
    if tomato_boxes == None:
        ...
        for tomato_box in tomato_boxes:
            ...
            # expected = 300
            # aspect = width / height
            # resized_image = cv.resize(color_image, (round(expected * aspect), expected))
           ...
           #  frameset1 = align.process(frameset1)
           #
           #  # Update color and depth frames:
           #  aligned_depth_frame = frameset1.get_depth_frame()
           ...
           #  print("Detected a {0:.3} meters away".format(dist))
            ...
            # depth_list = []
            # while True:
            #     depth = get_distance((m,n),frameset)#(i,j))
            #     cv.circle(result_image, (m,n), 4, (0, 255, 255), 5)
            #     # depth = 277
            #     depth = 1
            #     print('depth:', depth)
...
def getDist(xmin, ymin,xmax, ymax):
    ...
    #
    # net = cv.dnn.readNetFromCaffe("C:/Users/123456/Tompabotti/MobileNet-SSD/voc/MobileNetSSD_deploy.prototxt",
    #                                "C:/Users/123456/Tompabotti/MobileNet-SSD/voc/MobileNetSSD_deploy.caffemodel")
    ...
    #
    # blob = cv.dnn.blobFromImage(crop_img, inScaleFactor, (expected, expected), meanVal, False)
    ...
    # print("Detected a {0} {1:.3} meters away. and xmin={2} xmin_depth={3} xmax={4} xmin={5} xmax_depth={6}".format(
    #    className, dist, xmin, xmin_depth, xmax, xmax_depth, 1))
    return float(dist)
