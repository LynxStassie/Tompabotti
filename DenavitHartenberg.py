...
def DenavitHartenbergUR(theta, UR):
    ...
    # Initialize the parameters for UR5
        ...
        alpha = [np.pi / 2, 0, 0, np.pi / 2, -np.pi / 2, 0]
        T06 = DenavitHartenbergMatrix(a[0], d[0], alpha[0], theta[0])  # Initialize with the first
        # Initialize the parameters for UR5e
    elif (UR == "UR5e"):
        a = [0, -0.425, -0.39225, 0, 0, 0]
        d = [0.1625, 0, 0, 0.1333, 0.0997, 0.0996 + 243.3 / 1000]
        alpha = [np.pi / 2, 0, 0, np.pi / 2, -np.pi / 2, 0]
        T06 = DenavitHartenbergMatrix(a[0], d[0], alpha[0], theta[0])  # Initialize with the first
...
def InvKinematic(x, y, z, rx, ry, rz):
    ...
    for i in range(0, len(cl)):
        ...
        th[5, c:c + 2] = np.arctan2(
            (-invdespos[1, 0] * np.sin(th[0, c]) + invdespos[1, 1] * np.cos(th[0, c])) / np.sin(th[4, c]),
            (invdespos[0, 0] * np.sin(th[0, c]) - invdespos[0, 1] * np.cos(th[0, c])) / np.sin(th[4, c]))
